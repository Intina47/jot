...
func TestAssistantMemoryStoreCandidate_NormalizesCurrentProjectShape(t *testing.T) {
	mem := NewAssistantMemoryAt(filepath.Join(t.TempDir(), assistantMemoryFileName))
	now := time.Date(2026, 4, 9, 13, 0, 0, 0, time.UTC)

	first, ok, err := assistantMemoryStoreCandidate(mem, assistantMemoryConsolidationItem{
		Kind:        "project",
		Scope:       "user",
		Subject:     "Jot memory",
		Key:         "current_main_project",
		Summary:     "User's current main project is Jot memory",
		Value:       "The user is currently focused on a project called 'Jot memory'.",
		Evidence:    "my current main project is Jot memory right now",
		SourceType:  "user_input",
		Confidence:  "high",
		Verification:"user_confirmed",
		Bucket:      "active",
		ShouldStore: true,
	}, "my current main project is Jot memory right now", now)
	if err != nil || !ok {
		t.Fatalf("first assistantMemoryStoreCandidate error = %v, ok=%v", err, ok)
	}
	if first.Key != "main_project" || first.Value != "Jot memory" {
		t.Fatalf("expected normalized first project fact, got %#v", first)
	}

	second, ok, err := assistantMemoryStoreCandidate(mem, assistantMemoryConsolidationItem{
		Kind:        "project",
		Scope:       "user",
		...
		Evidence:    "actually my current main project is Jot env now | You are currently working on the Jot env project.",
		SourceType:  "assistant_turn",
		Confidence:  "high",
		Verification:"verified",
		Bucket:      "active",
		ShouldStore: true,
	}, "what project am i working on right now?", now.Add(5*time.Minute))
	if err != nil || !ok {
		t.Fatalf("second assistantMemoryStoreCandidate error = %v, ok=%v", err, ok)
	}
	if second.Key != "main_project" || second.Value != "Jot env" {
		t.Fatalf("expected normalized second project fact, got %#v", second)
	}

	facts := mem.Facts()
	if len(facts) != 2 {
		t.Fatalf("expected expired old project and active current project, got %#v", facts)
	}
	activeCount := 0
	expiredCount := 0
	for _, fact := range facts {
		...
		case MemoryBucketActive:
			...
			if fact.Value != "Jot env" {
				t.Fatalf("expected active project to be Jot env, got %#v", fact)
			}
		case MemoryBucketExpired:
			expiredCount++
			if fact.Value != "Jot memory" {
				t.Fatalf("expected expired project to remain Jot memory, got %#v", fact)
			}
		}
	}
	if activeCount != 1 || expiredCount != 1 {
		t.Fatalf("expected one active and one expired project fact, got %#v", facts)
	}
}
